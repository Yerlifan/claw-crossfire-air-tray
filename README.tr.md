...
Debian/Ubuntu'da tepsi arka ucu için: `sudo apt install python3-gi gir1.2-ayatanaappindicator3-0.1`; GNOME'da ayrıca *AppIndicator* eklentisini açın. Kaldırmak için `./linux/kurulum.sh -u`. Ayarlar `~/.config/clawtray/` altında.
...
Ayar flash'ında her tek baytlık alan `(değer, 0x55 eksi değer)` çifti olarak, çok baytlı gruplar sonunda `0x55 eksi toplam` ile saklanır:
